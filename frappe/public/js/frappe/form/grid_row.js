...
export default class GridRow {
	...
	make() {
		...
		this.row = $('<div class="d-flex flex-row"></div>')
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			this.row_check = $(
				`<div class="row-check sortable-handle"  style="width:25px">
					${this.row_check_html}
				</div>`
			).appendTo(this.row);

			this.row_index = $(
				`<div class="row-index sortable-handle" style="width:25px">
					<span>${txt}</span>
				</div>`
			)
				.appendTo(this.row)
			...
			this.row_check = $(`<div class="row-check search"></div>`).appendTo(this.row);
	...
	add_column_configure_button() {
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col d-flex justify-content-center" style="cursor: pointer; width:25px ">
	...
	validate_columns_width() {
		...
		if (total_column_width && total_column_width > 101) {
	...
	setup_columns() {
		...
		if (this.show_search) {
			// last empty column
			$(`<div class="col grid-static-col search"  style="width:25px"></div>`).appendTo(this.row);
		}
	}
	...
	make_search_column(df, colsize) {
		...
		let $col = $(
			'<div class="col grid-static-col p-' + colsize + ' search"  style="width:25px"></div>'
		).appendTo(this.row);
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $(
			'<div class="col grid-static-col p-' + 2 + " " + add_class + '" style="width:25px"></div>'
		)
